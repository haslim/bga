  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen animate-in fade-in">
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
                  <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-xl">
                      ...
                      <button onClick={() => setIsModalOpen(false)} className="p-1 hover:bg-white rounded-full transition"><X className="w-5 h-5 text-slate-500" /></button>
                  </div>
                  ...
                  <div className="p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                      <div className="grid grid-cols-3 gap-4">
                        <div className="col-span-2">
                            ...
                            <input 
                                type="text" 
                                className="w-full border border-slate-300 bg-white text-slate-900 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                      ...
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">İçerik / Notlar</label>
                          <textarea 
                              className="w-full border border-slate-300 bg-white text-slate-900 rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-500 text-sm min-h-[300px] font-mono"
                      ...
                      <div>
                        ...
                        <div className="border border-slate-300 bg-white rounded-lg p-2 flex flex-wrap gap-2">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Bilgi Bankası</h1>
          <p className="text-slate-500 mt-1">İçtihatlar, dilekçeler ve kurumsal hafıza</p>
        </div>
        <button 
          ...
          <div className="lg:col-span-1 space-y-2">
              <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm sticky top-24">
                  ...
                  <button 
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition flex justify-between items-center ${selectedCategory === 'ALL' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                      Tümü
                      <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{knowledgeBase.length}</span>
                  </button>
                  {categories.map(cat => (
                      <button 
                        key={cat}
                        onClick={() => setSelectedCategory(cat)}
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition flex justify-between items-center ${selectedCategory === cat ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}
                      >
                        {cat}
                        <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{knowledgeBase.filter(k => k.category === cat).length}</span>
                      </button>
                  ))}
              </div>
          </div>
          ...
          <div className="lg:col-span-3">
              ...
              <div className="grid grid-cols-1 gap-6">
                  {filteredEntries.map(entry => (
                      <div key={entry.id} className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition group">
                          <div className="flex justify-between items-start mb-3">
                              <div className="flex items-center gap-3">
                                  <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wide border 
                                      ${entry.category === 'İçtihat' ? 'bg-purple-50 text-purple-700 border-purple-100' : 
                                        entry.category === 'Dilekçe' ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                                      {entry.category}
                                  </span>
                              ...
                              <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition">
                                  <button onClick={() => handleOpenModal(entry)} className="p-2 hover:bg-blue-50 text-slate-400 hover:text-blue-600 rounded transition">
                                      <Edit3 className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => deleteKnowledgeEntry(entry.id)} className="p-2 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded transition">
                                      <Trash2 className="w-4 h-4" />
                                  </button>
                              </div>
                          </div>

                          <h3 className="text-lg font-bold text-slate-800 mb-2 group-hover:text-blue-700 transition">{entry.title}</h3>
                          
                          <p className="text-sm text-slate-600 mb-4 line-clamp-3 whitespace-pre-line leading-relaxed">
                              {entry.content}
                          </p>
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                              <div className="flex gap-2">
                                  {entry.tags.map((tag, idx) => (
                                      <span key={idx} className="text-xs bg-slate-50 text-slate-500 border border-slate-200 px-2 py-0.5 rounded flex items-center">
